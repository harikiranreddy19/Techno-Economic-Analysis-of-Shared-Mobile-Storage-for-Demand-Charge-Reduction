...
%%
% rng('default')
% EnerConAppend = [NaN*ones(NumUsers,96*3),EnerCon];
% EnerConDeter = zeros(NumUsers,96*7);
% EnerConStd = zeros(NumUsers,96*7);
% for user = 1:NumUsers
%     EnerConDeter(user,:) = mean(reshape(EnerConAppend(user,:),96*7,[])','omitnan');
%     EnerConStd(user,:) = std(reshape(EnerConAppend(user,:),96*7,[])',0,1,'omitnan');
% end
% EnerConDeterMat = repmat(EnerConDeter,1,118);
% EnerConStdMat = repmat(EnerConStd,1,118);
% error = normrnd(0,1,NumUsers,79296);
% EnerConDeterMat(:,1:(96*3)) = [];
% EnerConStdMat(:,1:(96*3)) = [];
% error(:,1:(96*3)) = [];
...
for k = 1:length(kappa)
    % Error = normrnd(0,kappa(k)*EnerCon,NumUsers,79008);
    Error = normrnd(0,kappa(k)*mean(EnerCon,2).*ones(NumUsers,79008),NumUsers,79008);
    EnerConForecast = EnerCon + Error;

for month = [1,2] 

for NumDrivers = 3                 % 1:5
...
ciParam.OrgEnerCon = EnerCon(1:NumUsers,MonthStartMet(month):MonthEndMet(month));
ciParam.OrgDC = ucParam.PiMaxUserPrime(:,1).*max(ciParam.OrgEnerCon,[],2) + ucParam.PiMaxUserPrime(:,2).*max(ciParam.OrgEnerCon(:,ucParam.PeakHours),[],2) + ucParam.PiMaxUserPrime(:,3).*max(ciParam.OrgEnerCon(:,ucParam.PartialPeakHours),[],2);
ciParam.EnerCon = EnerConForecast(1:NumUsers,MonthStartMet(month):MonthEndMet(month));
...
ModEnerCon = ciParam.OrgEnerCon - ciParam.UserCharLim.*u;
...
filename = ['MixedSetup','FineKappa',num2str(k),'NumDrivers',num2str(NumDrivers),'Month',num2str(month)];
save(filename)
drawnow
end
end
